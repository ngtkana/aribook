...
pub fn rank(a: &mut [Vec<Mint>]) -> usize {
    let h = a.len();
    let w = a[0].len();
    let mut rank = 0;
    for j in 0..w {
        if let Some(pivot) = (rank..h).find(|&pivot| a[pivot][j] != Mint::zero()) {
            a.swap(pivot, rank);
            let inv = a[rank][j].inv();
            a[rank].iter_mut().for_each(|x| *x *= inv);
            let (upper, center_lower) = a.split_at_mut(rank);
            let (center, lower) = center_lower.split_at_mut(1);
            let a_rank = &center[0];
            for v in upper
                .iter_mut()
                .chain(lower.iter_mut())
                .filter(|v| v[j] != Mint::zero())
            {
                for (x, y) in a_rank.iter().zip(v.iter_mut()) {
                    *y -= x * a_rank[j];
                }
            }
            rank += 1;
...
fn main() {
    ...
    let is_full_rank = (0..20).all(|_| {
        let x = rand::random::<i64>().into();
    ...
    println!("{}", if is_full_rank { "Yes" } else { "No" });
}

#[cfg(test)]
mod chap3_5_tutte_tests {
    use super::*;

    const BIN: &str = "chap3_5_tutte";

    fn test_sample(input: &str, output: &str) {
        proconcli::test_sample(input, output, BIN);
    }

    fn into_int_matrix(a: &[Vec<i64>]) -> Vec<Vec<Mint>> {
        a.iter()
    ...
    #[test]
    fn test_rank() {
        assert_eq!(rank(&mut into_int_matrix(&[vec![0, 0], vec![0, 0]])), 0);
        assert_eq!(rank(&mut into_int_matrix(&[vec![1, 0], vec![0, 0]])), 1);
        assert_eq!(rank(&mut into_int_matrix(&[vec![0, 1], vec![0, 0]])), 1);
        assert_eq!(rank(&mut into_int_matrix(&[vec![0, 1], vec![1, 0]])), 2);
        assert_eq!(rank(&mut into_int_matrix(&[vec![1, 1], vec![1, 0]])), 2);
    }
