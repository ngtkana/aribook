...
// }}}
use chap4_1_mobius_function_of_divisors::mobius_of_divisors;
use modint::ModTrait;
use proconio::input;
...
fn main() {
    input!(n: u64);
    let ans = mobius_of_divisors(n)
        .into_iter()
        .map(|(key, coeff)| Mint::from_i64(26).pow(n / key) * Mint::from_i64(coeff as i64))
        .sum::<Mint>();
    println!("{}", ans);
}

#[cfg(test)]
mod chap4_1_number_of_non_periodic_strings_tests {
    const BIN: &str = "chap4_1_number_of_non_periodic_strings";
